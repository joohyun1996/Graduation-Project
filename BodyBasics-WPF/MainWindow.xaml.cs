...
namespace Microsoft.Samples.Kinect.BodyBasics
{
    ...
    public partial class MainWindow : Window, INotifyPropertyChanged
    {
        ...
        public MainWindow()
        {
            ...
            my_serial = new SerialPort();
            isCanRead = false;
            //객체생성, 포트 8000에서 listening
            cli = new UdpClient();
                ...
                using (DrawingContext dc = this.drawingGroup.Open())
                {
                    ...
                    for (int i = 0; i < 6; ++i)
                    {
                        ...
                        if (body.IsTracked)
                        {
                            ...
                            //Debug.WriteLine("{0} => {1} : {2}", penIndex, jointPoints[JointType.HandLeft].X, jointPoints[JointType.HandLeft].Y);
                            //if (my_serial.IsOpen)
                            //{
                            //    int xPoint = (int)(jointPoints[JointType.HandLeft].X - jointPoints[JointType.Head].X);
                            //    int yPoint = (int)(jointPoints[JointType.HandLeft].Y - jointPoints[JointType.Head].Y);
                            //    string sendMsg = xPoint + " : " + yPoint;// + " : " + my_serial.ReadLine();
                            //    listBox_recvdata.Items.Add(sendMsg);
                            //    listBox_recvdata.SelectedIndex = listBox_recvdata.Items.Count - 1;
                            //    listBox_recvdata.ScrollIntoView(listBox_recvdata.SelectedItem);
                            //    //my_serial.WriteLine(sendMsg);
                            //}
            ...
            while (true)
            {
                if (my_serial.IsOpen && isCanRead)
                {
                    ...
                    catch
                    {
                        ...
                        listBox_recvdata.Dispatcher.BeginInvoke(new Action(Error));
                    }
                }
            }
        }
